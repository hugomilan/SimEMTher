function [] = nuniter (k, nres, cl, resul, malha, Tbster, nsdx, ...
...
for ares = 1:(nres-1); %varrendo os resul
    ...
    if ares*nkres < (cc+1)*ck
        ...
        for c = ((ares - 1)*nkres+1):ares*nkres
            ...
            Tbster = vunitergen(mt, malha, Tbster, nsdx, Tbterdk);
            ...
            %perdas por fluxo entre os meios.
            [malha, resul.(bres)] = cuniflux (malha, tt, resul.(bres), ceil((c-nr-1)/saltosimu)+1, ...
                    MFSB, MFNO, MFCON, 1);
                
                %perdas por fluxo interno. Mudança na refletida e na temp.
            [malha, resul.(bres)] = advecuniflux (malha, tt, resul.(bres), ceil((c-nr-1)/saltosimu)+1, ...
                ADSB, ADNO, ADCON, 1);
            
                %perdas por fluxo na fronteira. Mudança na refletida e na temp.
            [malha, resul.(bres)] = cunifroflu(malha, resul.(bres), tt,  FSBF, ...
                FNOF, FCOF, ceil((c-nr-1)/saltosimu)+1,1);
    ...
    if nres*nkres >= (cc+1)*ck
        ...
        for c = (cc*ck+1):k
            ...
            for bp = 1:nsdx
                ...
                [malha, resul.(bres)] = cunifroflu(malha, resul.(bres), tt,  tt, FSBF, ...
                    FNOF, FCOF, ceil((c-nr-1)/saltosimu)+1,1);
